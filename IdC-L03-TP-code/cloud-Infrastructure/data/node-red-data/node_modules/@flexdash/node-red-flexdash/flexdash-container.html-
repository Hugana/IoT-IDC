...
<script type="module">
  ...
  RED.nodes.registerType('flexdash container', {
    ...
    defaults: {
      ...
      min_cols: { value: 1, validate(v) { return (v>0 && v<=20) } },
      max_cols: { value: 20, validate(v) { return (v>0 && v<=20) } },
      unicast: {
        value: 'ignore',
      ...
      solid: { value: false },
      cols: { value: 1, validate(v) { return v>0 && v<=20 } },
      rows: { value: 1, validate(v) { return v>0 && v<=100 } },
    },
    ...
    oneditprepare() {
      try {
        ...
        function select_grid_tab(kind) {
          ...
          $("#grids-only,#panels-only,#panel-only,#subflowpanel-only").hide()
          $(`#${kind.toLowerCase()}-only`).show()
          if (kind.endsWith('Grid')) $(`#grids-only`).show()
        ...
        // handle buttons for _fd_socket choices
        function select_unicast(val) {
          $(".socket-button-group").removeClass("selected")
          $(`.socket-button-group[v=${val}]`).addClass("selected")
          $('#node-config-input-unicast').val(val)
        }
        $(".socket-button-group").on("click", function() {
          select_unicast($(this).attr("v"))
        })
        select_unicast(this.unicast||"disallow")
    ...
    oneditsave() {
      try {
        ...
        if (kind == 'SubflowPanel' || kind.endsWith('StdGrid')) {
...
<script type="text/html" data-template-name="flexdash container">
  ...
  <div class="form-row">
    ...
    <small class="fd-indent">Name shown above panel or grid in dashboard. Hidden if left empty.
      Change dynamically by sending <tt>msg.set_title</tt> to FlexDash ctrl node.
    </small>
  </div>
  <div class="form-row">
    ...
    <span class="button-group" id="kind-span">
      <button type="button" class="red-ui-button toggle kind-button-group" kind="StdGrid"
        >grid</button
        ><button type="button" class="red-ui-button toggle kind-button-group" kind="PopupGrid"
        >pop-up grid</button
    ...
    <br>
    <small class="fd-indent">A <b>grid</b> is the primary container for widgets and panels.
      ...
      A <b>subflow panel</b> is a panel that holds all widgets of a subflow, it can only be used with
      nodes in a subflow.
    </small>
  </div>
  
  <!-- Fields specific to grids -->
  <div id="grids-only" style="display: none">
    <div class="form-row">
    ...
    <div class="form-row">
      ...
      <small class="fd-indent">Minimum and maximum number of grid columns shown in the grid.
        This can be used to constrain the reflow based on window width. The values can be changed
        dynamically by sending <tt>msg.min_cols</tt> and/or <tt>msg.max_cols</tt> to a
        FlexDash ctrl node.</small>
    </div>  
    <div class="form-row">
      <label for="node-config-input-tab"><tt>_fd_socket</tt></label>
      <span class="button-group">
        <button type="button" class="red-ui-button toggle selected socket-button-group" v="ignore"
          >ignore</button
          ><button type="button" class="red-ui-button toggle socket-button-group" v="allow"
          >allow</button
          ><button type="button" class="red-ui-button toggle socket-button-group" v="require"
          >require</button>
      </span>
      <br>
      <small class="fd-indent">Select how widgets in the grid should handle incoming messages
        that have <tt>msg._fd_socket</tt> set.        
    </div>
    <div class="form-row">
  ...
  <input type="hidden" id="node-config-input-unicast">

</script>

<script type="text/html" data-help-name="flexdash container">
  <p>Represents a grid or panel in FlexDash.
    Refer to the documentation for
    <a href="https://flexdash.github.io/docs/using-flexdash/core-concepts/">core concepts</a>
    around page layout as well as specifics of
    <a href="https://flexdash.github.io/docs/using-flexdash/grid-panel-layout/">grid and
    panel layout</a>.
  </p>
</script>
